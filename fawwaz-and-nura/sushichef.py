...
DATA_DIR = "chefdata"
COPYRIGHT_HOLDER = "Tatweer for Educational Services in collaboration with the MoE of Saudi Arabia"
LICENSE = get_license(licenses.CC_BY, 
        copyright_holder=COPYRIGHT_HOLDER).as_dict()
AUTHOR = "Tatweer for Educational Services in collaboration with the MoE of Saudi Arabia"
...
CHANNEL_THUMBNAIL = "https://yt3.ggpht.com/a-/AN66SAxsATQJTLuWc_XbpMy6HBDGyIFQCzbR2xY01w=s288-mo-c-c0xffffffff-rj-k-no"                                    # Local path or url to image file (optional)
...
class Grades(object):
    ...
    def load(self, filename, auto_parse=False):
        with open(filename, "r") as f:
            grades = json.load(f)
            for grade in grades:
                subject_obj = Subject(lang=grade["lang"])
                subject_obj.auto_generate_lessons(grade["lessons"])
...
# The chef subclass
################################################################################
class FawwazandNuraChef(JsonTreeChef):
    TREES_DATA_DIR = os.path.join(DATA_DIR, 'trees')

    def __init__(self):
        build_path([FawwazandNuraChef.TREES_DATA_DIR])
        super(FawwazandNuraChef, self).__init__()
...
[:400], #400 UPPER LIMIT characters allowed 
    ...
    def write_tree_to_json(self, channel_tree):
        scrape_stage = os.path.join(FawwazandNuraChef.TREES_DATA_DIR, 
